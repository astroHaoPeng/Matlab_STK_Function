function y = GetSatSixElementsDataFun(Sat,TimeOption)
%%  获取Sat->Classical Elements->(ICRF)的卫星六要素数据

%% 获取数据结构
SatData=Sat.DataProviders;
DataCollection=SatData.Item('Classical Elements').Group.Item('ICRF');
%% 设置时间状态
if(nargin>1)
...
Sat_Semimajor     =cell2mat(DataElements.GetDataSetByName('Semi-major Axis').GetValues);
Sat_Eccent        =cell2mat(DataElements.GetDataSetByName('Eccentricity').GetValues);
Sat_Inc           =cell2mat(DataElements.GetDataSetByName('Inclination').GetValues);
Sat_RAAN          =cell2mat(DataElements.GetDataSetByName('RAAN').GetValues);
Sat_Arg_Perige    =cell2mat(DataElements.GetDataSetByName('Arg of Perigee').GetValues);
Sat_True_Anomaly  =cell2mat(DataElements.GetDataSetByName('True Anomaly').GetValues);
Sat_Arg_Latitude  =cell2mat(DataElements.GetDataSetByName('Arg of Latitude').GetValues);
%% 处理数据
NLength=length(Sat_True_Anomaly);
y=zeros(NLength,6);
for i=1:NLength
